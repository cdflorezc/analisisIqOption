...
def obtener_divisas_activas(API):
    active_assets = API.get_all_open_time()
    active_currencies = [asset for asset in active_assets.keys() if asset.endswith('forex') and active_assets[asset]['open']]
...
def estrategia_rsi_macd(timeframe, amount, rsi_threshold=30, max_trades=5):
    ...
    while operaciones_realizadas < max_trades:
        active_currencies = obtener_divisas_activas(API)
...
estrategia_rsi_macd(timeframe, amount, rsi_threshold, max_trades)
